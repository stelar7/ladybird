...
ErrorOr<JS::NonnullGCPtr<Key>> convert_a_value_to_a_key(JS::Realm& realm, JS::Value input, Vector<JS::Value> seen)
{
    ...
    if (input.is_number()) {
        ...
        return Key::create(realm, Key::KeyType::Number, input.as_double());
    }
    ...
    if (input.is_object() && is<JS::Date>(input.as_object())) {
        ...
        return Key::create(realm, Key::KeyType::Date, ms);
    }
    ...
    if (input.is_string()) {
        ...
        return Key::create(realm, Key::KeyType::String, input.as_string().utf8_string());
    }
    ...
    if (input.is_object() && (is<JS::TypedArrayBase>(input.as_object()) || is<JS::ArrayBuffer>(input.as_object()) || is<JS::DataView>(input.as_object()))) {
        ...
        return Key::create(realm, Key::KeyType::Binary, data_buffer);
    }
    ...
    if (input.is_object() && is<JS::Array>(input.as_object())) {
        ...
        return Key::create(realm, Key::KeyType::Array, keys);
    }
