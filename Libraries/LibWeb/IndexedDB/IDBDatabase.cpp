...
WebIDL::ExceptionOr<GC::Ref<IDBObjectStore>> IDBDatabase::create_object_store(String const& name, IDBObjectStore::IDBObjectStoreParameters const& options)
{
    ...
    auto key_path = options.key_path.has_value() ? options.key_path.value() : Variant<Empty> {};

    // 5. If keyPath is not null and is not a valid key path, throw a "SyntaxError" DOMException.
    if (!key_path.has<Empty>() && !is_valid_key_path(key_path))
    ...
    if (database->has_object_store(name))
    ...
    if (auto_increment && ((key_path.has<String>() && key_path.get<String>().is_empty()) || key_path.has<Vector<String>>()))
        return WebIDL::InvalidAccessError::create(realm(), "Auto-increment is true and key path is not empty"_string);

    // 9. Let store be a new object store in database.
    auto store = database->create_object_store(name);

    //    Set the created object store's name to name.
    store.set_name(name);

    //    If autoIncrement is true, then the created object store uses a key generator.
    store.set_auto_increment(auto_increment);

    //    If keyPath is not null, set the created object store's key path to keyPath.
    if (!key_path.has<Empty>())
        store.set_key_path(key_path);

    // 10. Return a new object store handle associated with store and transaction.
    return IDBObjectStore::create(realm(), store, transaction);
}
