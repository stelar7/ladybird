...
bool supports_container([[maybe_unused]] String const& container)
{
    // FIXME: Check FFmpeg?
    return true;
}
...
Optional<Vector<Bindings::MediaKeySystemMediaCapability>> get_supported_capabilities_for_audio_video_type(KeySystem const& implementation, CapabilitiesType type, Vector<Bindings::MediaKeySystemMediaCapability> requested_capabilities, Bindings::MediaKeySystemConfiguration const& config, MediaKeyRestrictions restrictions)
{
    ...
    for (auto& capability : requested_capabilities) {
        ...
        auto mime_type = MimeSniff::MimeType::parse(content_type.to_utf8_but_should_be_ported_to_utf16());
        ...
        auto const& container = mime_type->essence();

        // 8. If the user agent does not support container, continue to the next iteration. The case-sensitivity of
        //    string comparisons is determined by the appropriate RFC.
        ...
        auto const& parameters = mime_type->parameters();

        // FIXME: 10. If the user agent does not recognize one or more parameters, or if any parameters are not valid
        //            per the relevant specification, continue to the next iteration.

        // 11. Let media types be the set of codecs and codec constraints specified by parameters. The case-sensitivity
        //     of string comparisons is determined by the appropriate RFC or other specification.
        auto media_types = parameters.get("codecs"sv).value_or({});
        ...
        // 14. If encryption scheme is non-null and is not recognized or not supported by implementation, continue to the next iteration.
        if (encryption_scheme.has_value() && !implementation.supports_encryption_scheme(*encryption_scheme))
            continue;

        // 15. If robustness is not the empty string and contains an unrecognized value or a value not supported by implementation,
        ...
        // 16. If the user agent and implementation definitely support playback of encrypted media data for the combination of
        //     container, media types, encryption scheme, robustness and local accumulated configuration in combination with restrictions:
...
bool is_persistent_session_type(Utf16String const& session_type)
{
    ...
    // NOTE: This is the parameter to this function.
...
Optional<ConsentConfiguration> get_supported_configuration_and_consent(KeySystem const& implementation, Bindings::MediaKeySystemConfiguration const& candidate_configuration, MediaKeyRestrictions& restrictions, URL::Origin const& origin)
{
    ...
    case Bindings::MediaKeysRequirement::Required:
        // FIXME: If the implementation does not support use of Distinctive Identifier(s) in combination with
    ...
    case Bindings::MediaKeysRequirement::Optional:
        ...
        break;
    case Bindings::MediaKeysRequirement::NotAllowed:
        // FIXME: If the implementation requires persisting state in combination with accumulated configuration and
    ...
    for (auto const& session_type : session_types) {
        ...
        // 2. If accumulated configuration's persistentState value is "not-allowed" and the Is persistent session type?
        // algorithm returns true for session type return NotSupported.
        ...
        // 3. FIXME: If the implementation does not support session type in combination with accumulated configuration
        //           and restrictions for other reasons, return NotSupported.

        // 4. If accumulated configuration's persistentState value is "optional" and the result of running the
        //    Is persistent session type? algorithm on session type is true, change accumulated configuration's
    ...
    // 18. If accumulated configuration's distinctiveIdentifier value is "optional", follow the steps for the first
    ...
    if (accumulated_configuration.distinctive_identifier == Bindings::MediaKeysRequirement::Optional) {
        // FIXME: 1. If the implementation requires use of Distinctive Identifier(s) or Distinctive Permanent Identifier(s)
        //           for any of the combinations in accumulated configuration:
        if (false) {
    ...
    // 22. Let consent status and updated restrictions be the result of running the Get Consent Status algorithm on
    //     accumulated configuration, restrictions and origin and follow the steps for the value of consent status from
    //     the following list:
    ...
    case ConsentStatus::InformUser:
        ...
        //        the information that Distinctive Identifier(s) and/or Distinctive Permanent Identifier(s) as appropriate
        //        will be used if the distinctiveIdentifier member of accumulated configuration is "required".
...
Optional<ConsentConfiguration> get_supported_configuration(KeySystem const& implementation, Bindings::MediaKeySystemConfiguration const& candidate_configuration, URL::Origin const& origin)
{
    ...
    while (supported_configuration.has_value() && supported_configuration->status == ConsentStatus::ConsentDenied) {
        // 1. Let supported configuration and, if provided, restrictions be the result of executing the
...
{
    if (key_system == "org.w3.clearkey"sv) {
        return make<ClearKeySystem>();
    }
